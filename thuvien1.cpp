...
void outPutFile(Book a[], char fileName[], char fileName2[], int n)
{
	...
	for(int i=0;i<n;++i){
		if(a[i].SLNguoiMuon>0){
			for(int j=0;j<a[i].SLNguoiMuon;++j){
				b[dem].NguoiMuon[0]=a[i].NguoiMuon[j];
				++dem;
			}
		}
	}
	...
	fflush(stdin);
	for(int i=0;i<dem;++i){
		strcpy(tenNguoiMuon, b[i].NguoiMuon[0].TenNguoiMuon);
				replace_spaces(tenNguoiMuon);
		fprintf(file2ptr, "%20s%8d%8d%8d%8d%8d%8d", tenNguoiMuon, b[i].NguoiMuon[0].NgayMuon.ngay, b[i].NguoiMuon[0].NgayMuon.thang, b[i].NguoiMuon[0].NgayMuon.nam, b[i].NguoiMuon[0].NgayTra.ngay, b[i].NguoiMuon[0].NgayTra.thang, b[i].NguoiMuon[0].NgayTra.nam);
	}
